...
{
	static const intmax_t	RECURSIVE_MAX=200;	//!< template ÇÃçƒãAè„å¿íl
...
#undef PRIME_NUMBER_RECURSIVE_LEVEL
	...
	struct is_prime_impl< 5, isSimpleFilter> { typedef detail::true_type type; };	// 5 ÇÕëfêî
	template<bool isSimpleFilter>
	struct is_prime_impl< 3, isSimpleFilter> { typedef detail::true_type type; };	// 3 ÇÕëfêî
	template<bool isSimpleFilter>
	struct is_prime_impl< 2, isSimpleFilter> { typedef detail::true_type type; };	// 2 ÇÕëfêî
