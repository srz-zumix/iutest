...
inline void DefaultXmlGeneratorListener::OnTestProgramStart(const UnitTest& test)
{
	IUTEST_UNUSED_VAR(test);
	FileOpen(m_output_path.c_str());
}

inline void DefaultXmlGeneratorListener::OnTestProgramEnd(const UnitTest& test)
{
	if( m_fp == NULL ) 
	{
		FileOpen(m_output_path.c_str());
...
inline void DefaultXmlGeneratorListener::OnReportTestCase(IFile* file, const TestCase& test_case)
{
...
inline void DefaultXmlGeneratorListener::OnReportTestInfo(IFile* file, const TestInfo& test_info)
{
...
inline bool DefaultXmlGeneratorListener::FileOpen(const char* path)
{
...
inline void DefaultXmlGeneratorListener::FileClose(void)
{
	if( m_fp == NULL ) return;
	m_fp->Close();
...
inline void	DefaultXmlGeneratorListener::OutputXmlCDataSection(IFile* file, const char* data)
{
	file->Printf("<![CDATA[");
	file->Write(data, strlen(data), 1);
	file->Printf("]]>");
}
inline void DefaultXmlGeneratorListener::OutputXmlAttribute(IFile* file, const char* name, const char* value)
{
	file->Printf("%s=\"", name);
...
inline ::std::string DefaultXmlGeneratorListener::EscapeXml(const char* str, bool is_attribute)
{
	::std::string msg;
	if( str != NULL )
	{
